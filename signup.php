    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            try {
                ...
                $success = 'Registration successful! Please <a href="login.php" class="text-green-600 dark:text-green-400 hover:underline">log in</a>.';
...
<head>
  ...
  <style>
    ...
    /* --- ENHANCED CENTER DIV STYLES BELOW --- */
    .glass-card {
    ...
    /* --- END ENHANCED CENTER DIV STYLES --- */
  </style>
</head>
<body className="font-sans bg-green-100 transition-colors duration-300">
  ...
  <script type="text/babel">
    ...
    function SignUpPage() {
      return (
        <section className="flex-1 text-gray-900 dark:text-white py-16 px-6 text-center relative section-shadow" style={{ minHeight: 'calc(100vh - 320px)' }}>
          <div className="plant-left plant-sway"></div>
          <div className="plant-right plant-sway"></div>
          {/* Floating leaves for effect */}
          ...
          <div className="max-w-md mx-auto glass-card content-div px-8 py-10 md:py-12 md:px-12">
            <h2 className="text-2xl font-bold mb-4 text-green-800 dark:text-green-200">Create Your FATA Account</h2>
            <p className="text-lg mb-6 dark:text-gray-300">Join thousands of farmers and start growing smarter today.</p>
            <?php if (!empty($error)): ?>
              <p class="text-red-500 dark:text-red-400 mb-4"><?php echo htmlspecialchars($error); ?></p>
            <?php elseif (!empty($success)): ?>
              <p class="text-green-600 dark:text-green-400 mb-4"><?php echo $success; ?></p>
            <?php endif; ?>
            <form method="POST" className="space-y-5">
              ...
              <div className="input-wrapper">
                <img src="https://img.icons8.com/ios-filled/24/15803d/new-post.png" alt="" className="input-icon" />
                <input
                  type="email"
                  name="email"
                  placeholder="Email"
              ...
              <button type="submit" className="btn-primary w-full signup-btn-animate">Sign Up</button>
            </form>
            <p className="mt-5 dark:text-gray-300">
              Already have an account? <a href="login.php" className="text-green-600 dark:text-green-400 hover:underline">Log In</a>
            </p>
          </div>
        </section>
      );
